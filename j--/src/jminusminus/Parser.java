...
public class Parser {
    ...
    private JMember memberDecl(ArrayList<String> mods) {
        ...
        if (seeIdentLParen()) {
            ...
            if (have(VOID)) {
                ...
                memberDecl = new JMethodDeclaration(line, mods, name, type, params, throwsIdentifiers, body);
            } else {
                type = type();
                if (seeIdentLParen()) {
                    // Non void method
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
        	...
        	JExceptionExpression expr = exeptionexpression();
            mustBe(SEMI);
    ...
    private JExceptionExpression exeptionexpression() {
    	int line = scanner.token().line();
    	mustBe(NEW);
    	mustBe(EXCEPTION);
    	mustBe(LPAREN);
    	mustBe(RPAREN);
        return new JExceptionExpression(line, "This is an exception");
    }
